...
class EmotionProcessor(object):
    ...
    def volumeProc(self):
        ...
        def train(self):
            emotionLables =[Excited, Neutral] #These will be expanded later
            classifier = MLPClassivier(solver='lbfgs', alpha=1e-5,
                hidden_layer_sizes=(2,2), random_sate=1)
            classifier.fit(extractedData, emotionLabels)
            return classifier
